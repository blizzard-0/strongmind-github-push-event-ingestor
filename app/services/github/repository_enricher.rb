module Github
  class RepositoryEnricher
    ...
    def call(github_id:, api_url:)
      ...
      return reuse(repository) if usable?(repository)

      payload = fetch_payload(api_url)
      validate_identity!(payload, github_id)
      repository = persist(repository, github_id:, api_url:, payload:)

      log(:info, "enrichment.succeeded", github_id:, status: :fetched, fetched: true)
      EnrichmentResult.new(status: :fetched, record: repository, reason: nil, fetched: true)
    ...
    def usable?(repository)
      repository &&
        repository.raw_payload.is_a?(Hash) &&
        repository.raw_payload.any? &&
        repository.enriched_at.present?
    end

    def reuse(repository)
      log(
        :info,
        "enrichment.skipped",
      ...
      EnrichmentResult.new(status: :reused, record: repository, reason: :cached, fetched: false)
    end
    ...
    def persist(repository, github_id:, api_url:, payload:)
      attributes = {
        name: payload["full_name"].presence || payload["name"],
        api_url:,
        raw_payload: payload,
        enriched_at: clock.call
      }

      return repository.tap { |record| record.update!(attributes) } if repository

      Repository.create!(attributes.merge(github_id:))
    rescue ActiveRecord::RecordNotUnique
      concurrent_repository = Repository.find_by!(github_id:)
      return concurrent_repository if usable?(concurrent_repository)

      concurrent_repository.tap { |record| record.update!(attributes) }
    end
